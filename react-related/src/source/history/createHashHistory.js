...
const HashPathCoders = { // 定义hash路径的类型
  ...
  slash: { // 斜杠开头的hash路径，比如：'#/some/path'。这是默认的hash路径类型
    ...
    decodePath: addLeadingSlash // 不会去掉首'/'
  }
};
...
function createHashHistory(props = {}) {
  ...
  const { getUserConfirmation = getConfirmation, hashType = 'slash' } = props; // 通过Router实例的props初始化导航前处理函数以及hash路径类型
  ...
  function getDOMLocation() { //将当前URL转化为location对象
    ...
    return createLocation(path); // 创建location，从创建方式也可以看出HashHistory对象的location没有state属性和key属性
  }
  ...
  function handleHashChange() {
    ...
    if (path !== encodedPath) { // 确保哈希路径的类型和规定的一致
      ...
      // 如果不需要强制导航且当前fullpath未发生变化则不做处理
  ...
  function revertPop(fromLocation) { // 回滚到之前的位置。一些情况存在BUG——比如当前位于window.history最后一个历史记录项，直接修改hash(链接、location.hash、地址栏输入)触发hashchange时会新增历史记录项，然后导致allPaths不同步,因此计算出的delta为错误的，从而go跳转失败而不会触发hashchange，进而导致下次hashchange事件被忽略。
    const toLocation = history.location; // 导航前URL的location

    // TODO: We could probably make this more reliable by
    // keeping a list of paths we've seen in sessionStorage.
    // Instead, we just default to 0 for paths we don't know.

    let toIndex = allPaths.lastIndexOf(createPath(toLocation)); // 导航前URLfullpath最近一次缓存的索引

    if (toIndex === -1) toIndex = 0; // 没缓存过则置为0

    let fromIndex = allPaths.lastIndexOf(createPath(fromLocation)); // 导航后URLfullpath最近一次缓存的索引
    ...
    if (delta) {
      forceNextPop = true; // history.go会触发hashchange事件(调用handleHashChange函数)，而回滚不需要触发导航，所以通过forceNextPop让handleHashChange忽略本次hash变更
  ...
  function createHref(location) { // 根据location解析出URL字符串
    ...
    let href = '';
    if (baseTag && baseTag.getAttribute('href')) { // 如果存在带有href attribute的base元素则获取当前URL中#之前的部分作为前置部分(基路径)
    ...
    transitionManager.confirmTransitionTo(
      ...
      ok => {
        ...
        if (hashChanged) { // fullpath发生过变化，此时会触发hashchange事件
          ...
          ignorePath = path; // 下面的pushHashPath方法调用后会变更hash路径从而触发hashchange事件，所以这里通过ignorePath让handleHashChange不再触发handlePop执行。
          pushHashPath(encodedPath); // 变更hash路径

          const prevIndex = allPaths.lastIndexOf(createPath(history.location)); // 导航前fullpath最近一次缓存的索引
          const nextPaths = allPaths.slice(0, prevIndex + 1); // 拷贝该元素以及之前的元素
  ...
  function replace(path, state) {
    ...
    transitionManager.confirmTransitionTo(
      ...
      ok => {
        ...
        const prevIndex = allPaths.indexOf(createPath(history.location)); // 获取导航前fullpath最近一次缓存的索引
  ...
  function listen(listener) { // 注册导航监听器和hashchange事件监听器
