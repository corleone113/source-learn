...
declare type PathToRegexpOptions = { // pathToRegexp方法的选项对象的类型
  ...
  strict?: boolean, // false表示末尾斜杠是可选的，即匹配时忽略末尾斜杠
  end?: boolean // true表示结尾也要匹配，相当于精确匹配
}
...
type Position = { x: number, y: number }; // 坐标类型
...
declare type RouterOptions = {
  ...
  ) => PositionResult | Promise<PositionResult>; // 支持异步滚动
}
...
declare type RouteConfig = { // 路由配置对象的类型
  ...
  alias?: string | Array<string>; // 路径别名，是和path一样格式的字符串，匹配时URL路径保持不变且实际匹配的是该路由，
  children?: Array<RouteConfig>; //该路由的子路由——嵌套路由配置
  beforeEnter?: NavigationGuard; // 该路由的前置钩子
  meta?: any; // 匹配时携带的元数据，由路由记录携带,通过matched数组获取匹配的路由记录。
...
declare type RouteRecord = { // 路由记录的类型
  ...
  props: boolean | Object | Function | Dictionary<boolean | Object | Function>; // 实际上只有最后一种是有效的
}
...
declare type Route = {
  ...
  name: ?string;
  hash: string;
  query: Dictionary<string>; // 查询参数
  params: Dictionary<string>; // 路径参数
